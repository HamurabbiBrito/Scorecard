...
export default function ProduccionForm({ areaId, areaName }) {
  ...
  const handleChange = (e) => {
    ...
    setFormData(prev => ({
      ...prev,
      [name]: name.startsWith('percent') || name.startsWith('target') || 
              ['oee', 'produccionKm', 'produccionKg', 'kgScrap', 'kmReventon', 
               'scrapCu', 'scrapSn', 'scrapTotal', 'yield', 'yieldTotal',
               'kmEnviadosRebobinados', 'kmRebobinados'].includes(name)
        ? Number(value) || 0
        : value
    }));
  };
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md max-w-7xl mx-auto">
      <h2 className="text-2xl font-semibold mb-4">Reporte de Producción - {areaName.toUpperCase()}</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          {/* Número SAP */}
          <div>
            <label className="block text-sm font-medium text-gray-700">Número SAP</label>
            <input
              type="text"
              name="numeroSap"
          ...
          {/* Producción Km */}
          <div>
            <label className="block text-sm font-medium text-gray-700">Producción Km</label>
            <input
              type="number"
              name="produccionKm"
          ...
          {/* Producción Kg */}
          <div>
            <label className="block text-sm font-medium text-gray-700">Producción Kg</label>
            <input
              type="number"
              name="produccionKg"
          ...
          {/* % Scrap */}
          <div>
            <label className="block text-sm font-medium text-gray-700">% Scrap</label>
            <input
              type="number"
              name="percentScrap"
          ...
          {/* OEE */}
          <div>
            <label className="block text-sm font-medium text-gray-700">OEE</label>
            <input
              type="number"
              name="oee"
              value={formData.oee}
          ...
          {/* Reventones */}
          <div>
            <label className="block text-sm font-medium text-gray-700">Reventones</label>
            <input
              type="number"
              name="reventones"
